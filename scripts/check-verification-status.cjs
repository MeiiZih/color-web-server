...
(async () => {
  ...
  try {
    for (const width of [320, 390, 1280]) {
      ...
      let verified = false, failed = false, delay = false, mailRequests = 0;
      ...
      await page.route('**/api/user/profile',async r=>{
        ...
        await r.fulfill({status:failed?503:200,json:failed?{message:'offline'}:profile()});
      });
      await page.route('**/api/user/email-verification/request',r=>{ mailRequests++; return r.fulfill({json:{success:true}}); });
      ...
      await page.locator('[data-verification-status]:not([aria-busy]) button:enabled').waitFor();
      delay=true;
      await page.getByRole('button',{name:'重新確認狀態'}).click();
      await page.getByRole('button',{name:'正在確認…',exact:true}).waitFor();
      await page.getByText(/已重新確認：目前尚未驗證/).waitFor();
      assert.equal(await page.locator('[data-check-verification]').evaluate(el=>el===document.activeElement),true);
      assert.equal(mailRequests,0,'Checking status must not send mail');
      delay=false;
      await page.getByRole('link',{name:'管理 Email 驗證'}).click();
      await page.getByRole('button',{name:'寄送驗證信',exact:true}).waitFor();
      assert.match(page.url(),/account.html#profile$/);
      ...
      assert.equal(await page.getByRole('button',{name:'重新確認狀態'}).isEnabled(),true);
      failed=false;
      page.on('dialog',d=>d.accept());
      await page.goto('http://127.0.0.1:4180/app/#me');
      await page.getByText('✓ 已驗證',{exact:true}).waitFor();
      await page.getByRole('button',{name:'重新確認狀態'}).click();
      await page.getByText('已重新確認：你的 Email 已完成驗證。',{exact:true}).waitFor();
