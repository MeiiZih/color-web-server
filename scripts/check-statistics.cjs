...
(async()=>{
  ...
  try{for(const width of [320,390,768,1280]){
    ...
    assert.equal(await page.locator('.statistics-pie').count(),3);
    assert.equal(await page.locator('.statistics-mbti svg path').count(),5);
    assert.equal(await page.locator('.statistics-surveys svg circle').count(),1);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'no horizontal overflow');
    for(const item of await page.locator('.statistics-legend li').all()){
    ...
    await page.locator('.statistics-breakdown summary').click();
    assert.equal(await page.locator('.statistics-breakdown li').count(),9);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'expanded breakdown stays in viewport');
    ...
    if(width===390||width===1280)await page.screenshot({path:path.resolve(__dirname,`../test-results/statistics-pie-${width}.png`),fullPage:true});
    console.log(JSON.stringify({width,pies:3,overflow:false,labels:'readable',breakdown:9}));await page.close();
