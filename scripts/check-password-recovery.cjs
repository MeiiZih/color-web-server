...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{for(const width of [390,1280])for(const role of ['user','admin']){
 ...
 loginMode='success';await page.locator('#login-form [type=submit]').click();await page.waitForURL(role==='admin'?base+'admin':'http://127.0.0.1:4180/app/#me');assert(await page.evaluate(key=>!!sessionStorage.getItem(key),role==='admin'?'adminToken':'userToken'));assert(!calls.filter(c=>c.path.endsWith('/login')).some(c=>c.path!=='/api/'+role+'/login'));
