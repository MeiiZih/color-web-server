...
(async()=>{
  ...
  try{for(const width of [390,1280]){
    ...
    assert.equal(first.duration,320);assert.equal(first.focused,true);assert.equal(first.top,100);assert.equal(first.style,null);
    assert(first.frames[0].transform.includes('matrix(1, 0, 0, 1, 3, 0)'),'preserves existing transform');
    assert.equal(Number(first.frames[0].opacity),.86);
