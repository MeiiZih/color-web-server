...
(async()=>{
  ...
  try{for(const width of [390,1280]){
    ...
    assert.equal(first.duration,190);assert.equal(first.focused,true);assert.equal(first.top,100);assert.equal(first.style,null);
    assert(first.frames[0].transform.includes('matrix(1, 0, 0, 1, 3, 0)'),'preserves existing transform');
    assert.equal(Number(first.frames[0].opacity),.94);
    ...
    assert(await page.evaluate(()=>motion.commit('#home',{restored:true})===null),'retained pages restored without reveal');
    const quiz=await page.evaluate(()=>{const main=document.querySelector('main');main.style.transform='none';main.innerHTML='<div class="test-topline">作答頁</div><div class="quiz-companion">原稿角色</div><form id="question-form">第一題</form><div class="question-actions">下一題</div>';const controls=document.querySelector('.question-actions');const before=controls.getBoundingClientRect().bottom;const animation=motion.commit('#test/color');return {target:animation.effect.target.className,bottom:controls.getBoundingClientRect().bottom,before,mainAnimation:main.getAnimations().length,cast:document.querySelector('.quiz-companion').getAnimations().length};});
    ...
    await page.evaluate(()=>motion.destroy());await page.close();console.log(JSON.stringify({width,initial:'static',retained:'static',refresh:'static',quickNav:'cancels',quizFixedControls:'unchanged',reducedMotion:'static'}));
