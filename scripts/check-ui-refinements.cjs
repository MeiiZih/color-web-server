...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 for(const width of [320,390,768,1280]){
  ...
  assert(await page.locator('.first-use-link').evaluate((e,width)=>{const a=e.getBoundingClientRect(),p=e.parentElement.getBoundingClientRect();return width<768?Math.abs(a.x+a.width/2-p.x-p.width/2)<2:Math.abs(a.x-p.x)<2;},width));
  if(width>=768) for(const selector of ['.microcopy','.all-surveys-link'])assert(await page.locator(selector).evaluate(e=>getComputedStyle(e).textAlign==='left'||getComputedStyle(e).justifyContent==='flex-start'));
  if(width<768){assert.equal(await page.locator('.color-studio').evaluate(e=>getComputedStyle(e).marginTop),'0px');assert.equal(await page.locator('.footer-brand').evaluate(e=>getComputedStyle(e).textAlign),'center');assert.equal(await page.locator('.footer-companions').evaluate(e=>getComputedStyle(e).justifyContent),'center');}
  else assert.equal(await page.locator('.footer-brand').evaluate(e=>getComputedStyle(e).textAlign),'left');
  ...
  await page.locator('.hero').screenshot({path:`tmp/hero-alignment-${width}.png`});
  await page.locator('.organized-footer').screenshot({path:`tmp/footer-alignment-${width}.png`});
