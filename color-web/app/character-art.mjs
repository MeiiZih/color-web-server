...
export function character(key) {
  ...
  return `<span class="report-character articulated character-${key}" role="img" aria-label="${c.name}色報告角色" style="--character-ratio:.9;--character-delay:${-['red','yellow','green','blue'].indexOf(key)*3}s"><svg class="character-art" viewBox="${(c.width-viewportWidth)/2} 0 ${viewportWidth} ${c.height}" aria-hidden="true" focusable="false"><defs><clipPath id="${id}-head"><rect width="${c.width}" height="${c.split}"/></clipPath><clipPath id="${id}-legs"><rect y="${c.split}" width="${c.width}" height="${c.height-c.split}"/></clipPath></defs>${key==='yellow'?`${img}${lids}`:`<g clip-path="url(#${id}-legs)">${img}</g><g class="character-head" style="transform-origin:${c.width/2}px ${c.split}px"><g clip-path="url(#${id}-head)">${img}</g>${lids}</g>`}</svg></span>`;
}
