...
function authPage(admin = false) {
  return `<div class="auth-layout"><section class="auth-story"><span class="eyebrow">A LITTLE CLOSER TO YOU</span><h1>每一面，<br>都值得被理解。</h1><p>留一點時間給自己。<br>從一場色彩探索，重新認識你的模樣。</p><div class="color-marks" aria-hidden="true"><i></i><i></i><i></i><i></i></div></section><section class="panel auth-panel">${back('/app/#home', '回到首頁')}<h2>${admin ? '管理員登入' : '歡迎回來'}</h2><p class="hint">${admin ? '使用管理員帳號進入管理工作室。' : '登入後，讓每一次探索都有跡可循。'}</p><form id="login-form" class="form-stack">${field('email', admin ? '管理員帳號' : '電子郵件或管理員帳號', '', 'type="text" autocomplete="username" required')}${password()}${status}${submit(admin ? '登入管理工作室' : '登入')}</form>${admin ? '<a class="auth-switch" href="#login">回到會員登入</a>' : `<div class="auth-links">${link('/app/#surveys', '先以訪客探索')}${link('#register', '建立帳號')}</div><a class="auth-switch" href="#admin-login">管理員登入</a>`}</section></div>`;
}
