...
function contentEditor(item = {}) { return `${back('#content','首頁資訊')}${intro(item._id?'編輯資訊':'新增資訊')}<form id="content-form" class="panel form-stack">${select('type','顯示區域',[['news','最新資訊'],['common','一般資訊']],item.type||'news')}${field('title','標題',item.title,'required')}${area('description','內容說明',item.description,'required')}${field('link','外部連結（選填）',item.link)}${contentMetadata(item)}${field('imageUrl','圖片網址',item.imageUrl,'required')}<p class="hint">發布前需先生成並部署專屬插圖。請填入已登錄的 /assets/images/posts/ 圖片路徑；每週自動清單會先備妥圖片供審核，未完成插圖將無法儲存發布。</p>${status}<div class="actions form-actions">${submit('儲存資訊')}${link('#content','取消')}</div></form>`; }
...
function information(route) {
  if(route==='install') {
    ...
    return `<article class="prose">${back('/app/#me','我的空間')}${intro('把 ColorLab 放進你的日常','從手機主畫面開啟，少一點瀏覽器的干擾。')}<section class="panel"><h2>${standalone?'目前以獨立 App 模式開啟':'目前以瀏覽器模式開啟'}</h2><p>${standalone?'已偵測到獨立模式。手機的時間、電量與底部操作指示仍由系統顯示。':'目前仍可能顯示網址列及瀏覽器工具列。網站無法直接隱藏這些系統介面，請按下列方式重新加入主畫面。'}</p></section><section><h2>iPhone／iPad</h2><ol><li>用 Safari 開啟 ColorLab 網站。</li><li>開啟分享選單，選「加入主畫面」。</li><li>若看到「以網頁 App 開啟／Open as Web App」，請保持開啟，再按加入。</li><li>回到主畫面，點新加入的 ColorLab 圖示開啟，回來這裡確認模式。</li></ol><p>先保留舊捷徑；確認新版正常後再自行移除舊捷徑。不要清除網站資料，以免遺失訪客紀錄與草稿。不同安裝入口可能需要重新登入。</p><a href="https://support.apple.com/guide/iphone/open-as-web-app-iphea86e5236/ios" target="_blank" rel="noopener noreferrer">Apple 官方安裝說明 ↗</a></section><section><h2>Android／電腦</h2><p>使用 Chrome 或 Edge 開啟 ColorLab 網站，從瀏覽器選單選擇安裝應用程式或加入主畫面，再由新圖示開啟。名稱依瀏覽器版本而異。</p></section><p class="hint">原始後端服務仍處理登入與測驗資料，不需另行開啟或安裝。</p></article>`;
  }
